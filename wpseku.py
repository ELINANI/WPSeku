...
import os, sys, time, getopt, json
...
class WPSeku(object):
	...
	def usage(self,ext=False):
		...
		print "\t-o --output\tSave output to file as JSON, (df=None)"
		print "\t-w --wordlist\tSet wordlist, (df=db/wordlist.txt)"
		print "\t-h --help\tShow this help and exit\n"
		print "Examples:"
		print "\t{} -t http://site.com".format(path)
		print "\t{} -t http://site.com -o ./data.json".format(path)
	...
	# Empty object for collecting scan results
	result = type('', (), {})()
	...
	def main(self,kw):
		...
		# Parse process args
		try:
			opts,args = getopt.getopt(kw,"t:u:p:c:a:m:w:o:frbh:",["target=","brute",
				"user=","proxy=","cookie=","timeout=","agent=","ragent","redirect","wordlist=","output=","help"]
				)
		...
		fingerprint.fingerprint(agent=self.agent,proxy=self.proxy,
			redir=self.redirect,time=self.timeout,url=self.target,
			cookie=self.cookie,result=self.result).run()

		# Run discovering modules
		discovery.discovery().run(agent=self.agent,proxy=self.proxy,
			redir=self.redirect,time=self.timeout,url=self.target,
			cookie=self.cookie,result=self.result)

		# Print output to file
		if self.output:
			with open(self.output, 'w') as outfile:
				json.dump(self.result.__dict__, outfile, indent=2, sort_keys=True)
